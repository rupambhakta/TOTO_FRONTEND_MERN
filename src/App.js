...
export default function App() {
  ...
  const [filterPriority, setFilterPrioriy] = useState("all");

  // Function to fetch tasks from the database
  const fetchTask = async (token) => {
    const response = await fetch("https://todobackendmern.onrender.com/tasks", {
  ...
  // Calling the fetchTasks function using useEffect
  useEffect(() => {
  ...
  const deleteTask = async (id) => {
    await fetch(`https://todobackendmern.onrender.com/tasks/${id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  const updateTaskStatus = async (id, currentStatus) => {
    ...
    const response = await fetch(
      `https://todobackendmern.onrender.com/tasks/${id}/status`,
      {
        method: "PATCH",
        headers: {
          "Content-type": "application/json",
    ...
    const updateedTask = await response.json();
    setTasks(tasks.map((task) => (task._id === id ? updateedTask : task)));
  };

  //Update priority
  const updateTaskPriority = async (id, newPriority) => {
    const response = await fetch(
      `https://todobackendmern.onrender.com/tasks/${id}/priority`,
      {
        method: "PATCH",
        headers: {
          "Content-type": "application/json",
